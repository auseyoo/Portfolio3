...
<script type="text/javascript">
	...
	function fnSavePrmtAgenRqst(){
	    ...
	    var selBhfcClsDt = $("#bhfcClsDt").val();
	    selBhfcClsDt = new Date(selBhfcClsDt.substring(0, 4), Number(selBhfcClsDt.substring(4, 6)) -1, Number(selBhfcClsDt.substring(6, 8)));
		
	 	// 저장/전송 가능한 날짜인지 체크 - 선택한 월의 최대 대리점주문일과 현재 날짜 비교하기
		if(today > selBhfcClsDt){
		...
		$.ajax({
			url : "/prmt/savePrmtAgenRqstBhfc.do",
			type : 'POST',
		...
		$.ajax({
			...
			success : function(data) {
				if(data.list != null){
					...
					$("#bhfcClsDt").val(data.list.bhfcClsDt);
				}else{
					$("#prmtRqstSeq").val('');
					$("#dvyfgRqstMonth").val('');
					$("#bhfcClsDt").val('');
...
<div class="content">
	...
	<form id="frm" name="frm" method="POST">
		<input type="hidden" name="updPart" id="updPart" value="B">
		...
		<input type="hidden" id="bhfcClsDt" value="${rqstMonth.bhfcClsDt}">	
		<!-- 조회 -->
