...
public class CodeSupport2 extends EscapeBodyTag {
	...
	private String name; 		// 만들어질 select name
	private String all; 		// value = ""  포함 여부
	private String allNm; 		// 위에꺼에 라벨
	...
	public int doStartTagInternal() {
		 ...
		 */
		try {
			List<CommCodeVO> result = commCodeService.selectCommCodeList(new CommCodeVO() {{
				setCommGrpCd(commGrpCd);
				setUseYn("Y");
			}});
			if(classNm == null ) classNm = "";
			String el = "<select name='" + name +"' id='"+name+"' class='"+classNm+"'>";
			if(all == null) {
				el += "<option value=''>전체</option>";
			}else if(all.equals("true")) {
				el += "<option value=''>"+allNm+"</option>";
			}
			for(CommCodeVO m : result) {
				
				el += "<option value='"+ m.getCommCd()+ "'>"+ m.getCommNm()+"</option>";
			}
			el += "</select>";
